...
describe('activeChips: grade band suppression', () => {
  ...
  // The landing hero's grade quick-links (#41) link to single grades (min === max), which the
  // range label would render as the nonsense "Grades 7–7".
  it('labels a single grade without a range (7..7)', () => {
    const chips = activeChips('/competitions', params({ minGrade: 7, maxGrade: 7 }));
    expect(chips.find((c) => c.key === 'grade')?.label).toBe('Grade 7');
  });
