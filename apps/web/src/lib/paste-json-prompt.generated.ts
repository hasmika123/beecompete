...
//
// Source: docs/seeding/paste-json-prompt.md (edit the prompt THERE).
...
export const PASTE_JSON_PROMPT_SOURCE = 'docs/seeding/paste-json-prompt.md';

/** The numbered workflow from the doc's "Instructions" section. */
export const PASTE_JSON_PROMPT_STEPS: readonly string[] = [
  "Copy the prompt below into any assistant (ChatGPT, Claude, whatever).",
  "Paste in the URL, page text, or your notes where it says so.",
  "Read the Notes — they come AFTER the JSON in the reply. §1 first: if it says the competition looks dead, stop and check before spending any more time on it.",
  "If the reply ends by offering to generate more FAQs or resources, answer it before you paste — topping up in the chat costs one message; typing rows into the form costs far more.",
  "Paste the JSON into Admin → Competitions → New → Paste JSON.",
  "Take the image prompt to ChatGPT, Gemini, or any image generator, generate the cover, and upload it in the form's Cover image control (5 MB max; PNG/JPEG/WebP). The field is required. The prompt writes the art direction only — sourcing the actual picture stays yours (restored 2026-09-01; the no-imagery rule below is unchanged).",
  "Swap each Amazon link from the Amazon-links note for its tag=beecompete-20 version in the Resources step, and tick \"affiliate\" on each one as you do — the tag is what triggers our disclosure obligation, and the model deliberately leaves the box unticked.",
  "Review the filled form and save.",
];
...
export const PASTE_JSON_STEP_SLICES: readonly StepPromptSlice[] = [
  ...
  },
  {
    ...
    tail: "AFTER the JSON, list every award the source actually names — the award title, then its cash value or what it is (a medal, a trip, a scholarship) — one per line, in the order the source presents them. That list has no key in the payload, so I type those rows in myself. Only awards the source states; no filling out a ladder that is not there.",
  },
  {
    id: "timeline",
    label: "Timeline",
