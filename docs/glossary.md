...
| **Key date** | One dated row on a Competition's timeline — the `key_date` table since `0002`, and the public "Key dates" panel. Carries a *type* (what kind of moment), an optional *label*, and its dates. | Canonical. Avoid "timeline entry", "date row", and especially **"milestone"** — that word is taken (see Compliance & minors). `starts_at` NULL = **date TBD** (`0008`), not "no date". |
| **Window** | A Key date with a start AND an end — registration, submissions, a multi-day event. It is a *period*, so it can open and close access. | One of the two **arities** a Key date type can have (`docs/timeline-model-plan.md` §4). A window with a blank start is legitimate: "closes Nov 3, open date not published". |
| **Point** | A Key date with a single date — results announced, an announcement. It is a *moment*, so it can notify but can never gate access. | The other arity. A point never carries an end date; under the plan that becomes a DB constraint, so "registration" typed as a point — a gate that opens and never shuts — is unrepresentable. |
...
| **Eligibility** | Who may enter — grade/age band, region, school type, other criteria. | Age rules may be anchored to an **age cutoff date** ("under 19 as of June 1"). Which of grade/age is the *stated* rule is recorded by **[Eligibility basis]** — see the row above. |
| **Eligibility basis** | Which axis the ORGANIZER actually states as the entry rule — `GRADE` · `AGE` · `BOTH` · `OPEN` (explicitly no restriction) · null (not stated). Spine column `eligibility_basis` (owner 2026-08-28). The stated axis is the one every summary surface renders; the other axis, when present, is a **derived** range we compute for filtering. | ⚠ **Never present a derived range as a stated rule.** Breakthrough Junior Challenge states *ages 13–18* and no grades — publishing "Grades 7–12" tells a 12-year-old in grade 7 they qualify when they do not. A derived range is a search aid, labeled as approximate wherever it shows. Null is **not** "all grades": absent eligibility is *unknown*, and we say **"Not stated"**. Refines the locked domain-model **Q2**, which keeps grade as the primary FILTER axis — basis governs DISPLAY. |
...
| **Blurb** | The two-line descriptive text on a CompetitionCard. **Derived, never authored** — the API truncates the Competition's `description` to 300 chars on a word boundary and sends it as `blurb`. | Replaced the curated `summary` field, retired 2026-08-21 (domain-model §7). Curators write one prose field, `description`, and the card takes its opening. |
...
| **Weekly Digest** | The free weekly email of *newly added and closing-soon competitions, curated by us* — **one send, identical for every subscriber** (rev R1-15c, owner 2026-07-18). Signup asks email in the band, then three **optional** preference questions (grade / interest / state) in a popup (owner 2026-07-26) — stored as Brevo attributes for curator insight + M26 segmentation, with **no effect on the R1 send**. The R1 email-capture product (→ R1-15, Brevo). | Canonical UI label. Avoid "newsletter", "subscription", "mailing list". **Was** personalized-by-preference (grade/interest/region); that promise moves to **M26, Phase 2**, when automated matching can honor it. Distinct from *Promotion* digest inclusion (paid placement inside this send, Phase 3+). |
| **Follow** (a Competition) | Subscribing **by email, with no account**, to updates about one Competition — we email when its dates are announced or updated (→ M29, R1-15b). | Canonical UI label ("Follow this Competition"). Distinct from **Save**, the account-based Tracker action at R2 (M7). Not a "subscription" in UI copy. Deliberately does **not** promise per-deadline reminders — that's M30/X11 in Phase 2. |
| **Host Waitlist** | The email list of **Hosts wanting early access to host tools**, not tied to any one listing (→ H46). Captured on the Landing `#hosts` band and as an opt-in on a Claim Request. | Canonical term. Distinct from a *registration* waitlist (H4 — participants queuing for a full Edition); never shorten either to just "waitlist" in code or docs. |
| **Claim Request** | A Host's request to take ownership of a listing we seeded: a **form that emails our admin inbox**, not a list subscription (→ H46, R1-15c). | Canonical term for the R1 intake. Distinct from **Claim** itself (the verified ownership transfer, H1 + Host Verification DQ11, Phase 3). A Claim Request is a support conversation; it must never be answered by adding someone to a mailing list. |
| **Entry fee** | What it costs to enter — the canonical UI label everywhere the price is shown or edited (owner 2026-08-28): the details page's At-a-glance strip and Logistics tab, the marketplace **Entry fee** facet, the admin create/import form, and the correction-request field picker. Free/Paid is a *value* of this field, not a field of its own. | Canonical UI label. ⚠ **Never label it "Cost"** — that was the pre-2026-08-28 label and is retired. Code and wire names are unchanged and stay as they are: `Competition.cost_type` / `costType`, `Edition.entry_fee` / `entryFee`, and the marketplace `?cost=free|paid` query param (a public URL contract — renaming it would break shared links). The card shows the value alone (Free / Paid) with no label. |
| **Logistics** (tab) | The competition-details tab answering *how this is run and how you sign up*: sign-up link, entry fee, delivery, location, **Scope**, recurrence, format, team size, contact email/phone (→ #108, 2026-08-26). A UI label, not an entity. | Canonical UI label. ⚠ **Never call this tab "Administration"** even though it mirrors the admin form's `administration` step 1:1 — **Admin / Curator** is reserved for internal staff (§People), so "Administration" on a public page reads as staff-facing. |
| **More** (tab) | The competition-details **overflow bin** tab: the Category Template attributes no designed tab claimed, plus curator **Tags** (→ #108, 2026-08-26). A UI label, not an entity. | Canonical UI label. Avoid "Details" — #87 retired that tab name when it split into Eligibility + Judging; reusing it resurrects a dead label with different contents. Fields graduate OUT of More as they earn a designed home. |
| **Scope** (of an Edition) | How far one running reaches — International · National · Statewide · Regional · Local · Online (no fixed region). Stored as `Edition.scope_level`. | Public since #108 (the Logistics tab); previously curated but never rendered. Distinct from **Region** (the specific places an Edition runs in, `EditionRegion`) and from **Delivery** (in person / online / hybrid). An International running is tagged with the **`International / Worldwide`** region row (Liquibase `0025`, 2026-09-03) — the registry seeds US geography only, so that row is how "many countries" is said without naming them. |
...
| **Listing Status** | The lifecycle axis of a Listing: **draft (optional) → published ⇄ unlisted → archived**, plus **in-review** — since 2026-08-25 the curator submit-for-review state on the unified `/admin/review` queue (Phase 3 reuses it for unverified self-submissions, DQ12). `listing_status` column (built 2026-08-25, migration `0021`). | Distinct from the participant **Journey** (a student's saved→…→result) and from **Visibility**. Supersedes the prototype's `DRAFT→PENDING_APPROVAL→ACTIVE→PAUSED→…` (legacy-reference §1). |
...
| **Verification / Trust Ladder** | The **Organization's** trust state: **Curated (unclaimed) → Claimed (unverified) → Verified** — verified implies claimed; "Unverified" retired (owner 2026-07-13). Listings carry no trust state of their own: a competition's maintainer is **derived** from its organizer org (org claimed/verified ⇒ host-maintained; org curated or no org ⇒ curated). | Anti-scam; see registry DQ11–DQ14 + domain-model §3f. Surfaced as **maintainer attribution**: "Listing *maintained by* BeeCompete's Curation Team" (org unclaimed / no org; possessive rev 2026-08-26) → host org (claimed+). Never "managed by" — the organizer runs the competition; the maintainer keeps the *listing* accurate. The verified seal is org-level only. |
...
| Term | Definition | Notes |
|---|---|---|
| **Audience** | **Every level — elementary school through graduate school.** Canonical since 2026-08-25 (owner), replacing "K-12", which the catalog never actually matched. Say "every level", "all levels", or name the span; **do not** write "K-12 marketplace", "K-12 audience", or "K-12-directed" about BeeCompete itself. | ⚠ The phrase **"not directed to children under 13"** is a legal term of art, not a synonym — it appears in the Privacy Policy and drives the FTC's child-directed analysis. Use it only where that legal meaning is intended, and never as a claim that no under-13s are here: we expect them and build for them. "K-12" is still the RIGHT word for the market, for competitors' products, for a listing's own grade band, and for the Phase-3 science-fair wedge — it is only wrong as a description of our audience. |
...
| **Milestone** | A deadline-gated, approval-driven step in a participant's structured workflow, with visible status. | Registry HC7 (🛑 Gate B). ⚠ **The word is reserved for this.** It is NOT a synonym for a **[Key date]** — that is a dated row on a public timeline, with no approval, no status and no workflow. `timeline-model-plan.md` originally proposed renaming the `CUSTOM` key-date type to `MILESTONE`; this entry is why that rename was dropped (2026-08-30). Prompt prose and the curation form still use "milestone" loosely for a Key date — pre-existing, flagged, not yet reconciled. |
