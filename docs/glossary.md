...
| **Logistics** (tab) | The competition-details tab answering *how this is run and how you sign up*: sign-up link, cost, delivery, location, **Scope**, recurrence, format, team size, contact email/phone (→ #108, 2026-08-26). A UI label, not an entity. | Canonical UI label. ⚠ **Never call this tab "Administration"** even though it mirrors the admin form's `administration` step 1:1 — **Admin / Curator** is reserved for internal staff (§People), so "Administration" on a public page reads as staff-facing. |
