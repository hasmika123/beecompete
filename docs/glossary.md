...
| **Weekly Digest** | The free weekly email of *newly added and closing-soon competitions, curated by us* — **one send, identical for every subscriber** (rev R1-15c, owner 2026-07-18). Email is the only field collected. The R1 email-capture product (→ R1-15, Brevo). | Canonical UI label. Avoid "newsletter", "subscription", "mailing list". **Was** personalized-by-preference (grade/interest/region); that promise moves to **M26, Phase 2**, when automated matching can honor it. Distinct from *Promotion* digest inclusion (paid placement inside this send, Phase 3+). |
