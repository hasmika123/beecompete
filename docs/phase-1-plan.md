...
**R1 follow-ups — surfaced 2026-07-13 during the admin/marketplace UI review; owner deferred both (not for this pass):**
- **R1-18** — **Deadline "TBD" support.** Let a key date carry an unknown / to-be-determined date. Model: make `KeyDate.starts_at` **nullable** (additive Liquibase migration; null = TBD) — remove the entity `@NotNull` + relax the create-DTO validation. UI: a "Date TBD" toggle in the edition **key-date manager** that clears/omits the `datetime-local` value. Read path: `EffectiveStatus.compute` must **skip null-date key dates** (a TBD `REG_CLOSE` must NOT render a listing `CLOSED`, and must not appear in the `deadlineWithinDays` filter); the public detail timeline shows "TBD". Deadlines live on Editions (D3, timeline-as-data), never on Competition. *(extends R1-1/R1-7; owner asked 2026-07-13, deferred.)*
- **R1-19** — **Org trust ladder + derived competition maintainer.** Per owner (refined 2026-07-13): trust lives on the **Organization only**, as a ladder — `CURATED` (unclaimed; verification N/A) → `CLAIMED` (host claimed, unverified) → `VERIFIED` (claimed + identity verified; verified implies claimed). A **competition is never verified/unverified and is not individually claimed or curated** — its maintainer is **derived from the organizer org** (org claimed/verified ⇒ all its competitions host-maintained; org curated or no organizer ⇒ curated by BeeCompete); claiming an org claims all its competitions by derivation, no cascade writes. Realign: retire org `UNVERIFIED`; make Competition/Edition `verification_state` vestigial (constant `CURATED`, column kept — additive-only); remove the admin competition verification control + list column; redesign the detail trust panel around the derived maintained-by line + org seal. **Already done (2026-07-13):** the CompetitionCard dropped its competition-level trust/"unverified" badge (org seal retained). **Full design: `docs/sweep-remediation-plan.md` §A2** (incl. migration `0008` + a schema-independent Phase-0 web half). *(revises R1-9/DQ13; schema; owner asked 2026-07-13, deferred.)*
