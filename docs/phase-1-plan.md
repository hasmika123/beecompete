...
- **R1-19** — **Competition "approved/listed" vs "verified" split.** Per owner: a **competition** can only be admin-**approved** (published to the catalog) — it is never "verified"; **verification is an organization property only**. Today Competition *and* Edition reuse the shared 4-value `VerificationState` (incl. `VERIFIED`) — see `Competition.verificationState`, the detail-page `trust-panel.tsx` (renders the competition's tier as a badge), and the admin `competition-header-actions.tsx` verification dropdown. Realign: introduce a competition listing/approval concept distinct from org verification; stop the detail panel + admin control from marking a competition "verified"; keep the org verified seal. **Already done (2026-07-13):** the CompetitionCard dropped its competition-level trust/"unverified" badge (org seal retained). This task finishes the **model + detail-page + admin** realignment. *(revises R1-9/DQ13; schema; owner asked 2026-07-13, deferred.)*
