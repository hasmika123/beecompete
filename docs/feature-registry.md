...
| DQ4 | Duplicate competition detection & merging | 1 (detection) / 2 (merge) | L | **Detection slice BUILT 2026-09-03** (`duplicate-detection-plan.md`, migration `0026`): DB name/URL keys, live-name uniqueness, confirmable soft signals, form pre-check, queue flags + pending twins, seeding pre-check, organization gate. Mark-as-duplicate + redirect = plan PR 2; **content merge stays Phase 2** (`sweep-remediation-plan.md` §20). |
...
| H47 | **Award structures** — per-Edition award list (place, monetary/non-monetary/travel-grant, value + currency, display order) managed by hosts; assignment to winners lands with judging (Gate B) | 3 | M | 🪝 `Award` entity reserved in P1 (domain-model, Rev 7); typed prize summary on Edition ships R1 for display. **Owner asked 2026-08-17 for typed prize KINDS on the card** (cash-with-amount / scholarship / internship / bragging rights …) — planned as an extension of this item, sliced additively, in `sweep-remediation-plan.md` §15. Read that before scoping H47 |
